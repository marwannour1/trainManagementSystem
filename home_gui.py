...
class HomePage:
    ...
    def create_main_page(self):
        ...
        def refresh_Pbutton():
            self.controller.refresh_Ptable(previous_trips_table,stations,start_varP,destination_varP,date_varP)
        def refresh_Abutton():
            self.controller.refresh_Atable(available_trips_table,stations,start_var,destination_var,date_var)     
        def refresh_Cbutton():
            self.controller.refresh_Ctable(current_trips_table,stations,start_varC,destination_varC,date_varC)     
        def done_Atable():
            self.controller.done_Atable(available_trips_table,stations,start_var,destination_var,date_var)
        def done_Ptable():
            self.controller.done_Ptable(previous_trips_table,stations,start_varP,destination_varP,date_varP)
        def done_Ctable():
            self.controller.done_Ctable(current_trips_table,stations,start_varC,destination_varC,date_varC)

        header_label = ttk.Label(self.root, text="Welcome, User!", font=("Arial", 16, "bold"))
        ...
        refresh_button = ttk.Button(available_trips_frame, text="Reset", command=refresh_Abutton)
        ...
        refresh_buttonP = ttk.Button(previous_trips_frame, text="Reset", command=refresh_Pbutton)
        ...
        current_trips_table = ttk.Treeview(
            current_trips_frame, columns=("Start", "Destination", "Date"), show="headings"
        )
        ...
        refresh_buttonC = ttk.Button(current_trips_frame, text="Reset", command=refresh_Cbutton)
